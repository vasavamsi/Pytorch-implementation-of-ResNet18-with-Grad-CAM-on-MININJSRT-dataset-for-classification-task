...
#Dataloader
train_path = "./Gender01_RGB/train" #Change the path to training set directory
val_path = "./Gender01_RGB/val" #Change the path to validation set directory
test_path = "./Gender01_RGB/test" #Change the path to testing set directory
...
net.fc = nn.Linear(num_features, 2) #change final layers to have 4 dim output for 2 classes [male,female]
net = net.to(device)

test_count = len(glob.glob('./Gender01_RGB/test/**/*.png')) #replace the path to test data
checkpoint = torch.load('./MINIJSRT_ResNet18_Gender/MINIJSRT_ResNet18_Gender_10.model')  #replace the path to the checkpoint
...
classes = os.listdir('./Gender01_RGB/test')
print('targets :', y)
